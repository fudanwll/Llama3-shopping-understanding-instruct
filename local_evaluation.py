...
def main():
    ...
    DATA_FILENAME = "./data/development.json" #测试集路径

    if not os.path.exists(DATA_FILENAME): #报个测试集不存在的异常
    ...
    data_df = load_development_data(DATA_FILENAME)#加载json文件（测试集）
    ...
    model = UserModel()#加载模型

    # Generate model outputs
    df_outputs = generate_model_outputs(data_df, model) #指导模型进行预测输出
    
    # add outputs to the data_df
    merged_data_df = pd.merge(data_df, df_outputs, on="input_field") #对模型输出数据进行合并
        
    print(merged_data_df.head())

    # Evaluate the generated outputs and calculate metrics
    per_task_metrics = evaluate_outputs(merged_data_df) #对模型预测输出的每个任务进行评估指标和得分

    # Aggregate and display the evaluation scores
    overall_metrics = aggregate_scores(per_task_metrics)#对各类任务模型的回答进行打分
    print("=" * 100)
    ...
    overall_score = overall_metrics["overall_score"].mean() #取各类分数平均值
    print(f"Overall Score: {overall_score}") #输出成绩
